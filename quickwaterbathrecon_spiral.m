...
sppar.nl = nshot;
[~,k,tsp,~,~,NN] = spiralgradlx6(fov,dim,sppar.dt,sppar.slew/100,sppar.gmax,sppar.nl,1,1);
k = k(:);
for ii = 2:sppar.nl
...
% 
% figure; im([imgrec;imgrec2]); title(sprintf('fully sampled; undersampled by %d',accfactor));
% 
% 
% return
...
for jj = libInd+1:size(img,3) % loop over dynamics
    ...
    Grec = block_fatrix(Grec,'type','col');
    %[xS,info] = qpwls_pcg(0*mask(:),Grec,1,col(dtmp(:,1:accfactor:sppar.nl,jj)),0,0,1,niters,mask(:));
    [xS,info] = qpwls_pcg(0*mask(:),Grec,1,dsamp(:),0,0,1,niters,mask(:));
    imgrec2 = embed(xS(:,end),mask);
    ...
    acqp.data = permute(dsamp,[1 3 2]);%permute(dtmp(:,1:accfactor:sppar.nl,jj),[1 3 2]);%G*imghot;% - dataBath;     	    % k-space data samples
    acqp.fov = fov;             % field of view
    acqp.k = ksamp;%reshape(permute(ksamp,[1 3 2]),[473*12 2])%kmask;%ktmp;              % k-space sampling mask % WAG REDFLAG: This is not an actual mask, but instead is the k-space sample locs, which don't seem to be the same as the mask
    ...
    algp.lam = 10^-4.5;%10^-5;%10^-6;% 10^-3.5];     % sparsity regularization parameter
    algp.beta = 10^-20;%5;%6;%10^-5.5;%10^-5.75;%4.5;         % roughness regularization parameter
...
%     tcrecon(:,:,jj) = embed(xS(:,end),mask);
%     thetatcr(:,:,jj) = angle(imglib.*conj(tcrecon(:,:,jj)));%.*exp(1i*Ac(:,:,jj)));
%     
%     % CG mask
...
%     tcrecon_cgmask(:,:,jj) = embed(xS(:,end),mask);
%     thetatcr_cgmask(:,:,jj) = angle(imglib.*conj(tcrecon_cgmask(:,:,jj)));
...
save reconspiral_scalebathnoWav
