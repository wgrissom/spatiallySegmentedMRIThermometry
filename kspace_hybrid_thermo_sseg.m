function [theta,A,c,f,Ac,algp] = kspace_hybrid_thermo_sseg(acqp,algp)
...
%%% modified from kspace_hybrid_thermo_mask.m to use singular value soft  
%%% thresholding-based compressed sensing recon in waterbath (f_update_bath)
...
% build system matrix
[Gbrain,Gbath,Gall,GbathSENSE] = buildG(Nc,acqp.k,acqp.fov,acqp.mask,algp);
if Nc > 1
    Gbath = GbathSENSE;
end
...
% mask initial theta
thetainit = algp.thetainit(algp.maskbrain);
theta = thetainit;
...
fBrain = f_update_brain(acqp.data,Ac,theta,acqp.L(repmat(algp.maskbrain(:),[1 Nc]),:),Gbrain);
% initial bath baseline is zero
if algp.updateBath
   bathinit = algp.bathinit(algp.maskbath(:));
   % bathinit = algp.bathinit(repmat(algp.maskbath,[1 1 Nc]));
else
   % bathinit = acqp.L(algp.maskbath);
   if Nc > 1
       bathinit = ssq(reshape(acqp.L,[size(acqp.mask) Nc]),3);
       bathinit = bathinit(algp.maskbath);
   else
       bathinit = acqp.L(algp.maskbath);
   %bathinit = acqp.L(repmat(algp.maskbath(:),[1 Nc]),:);
   end
end
fBath = bathinit;
...
if isempty(algp.masknz) % only run if we don't already have a heating mask 
    ...
    while costOld-cost >= algp.stopThresh*costOld
        ...
        dataBath = acqp.data(:) - Gbrain*(fBrain.*repmat(exp(1i*(Ac+theta)),[Nc 1])); % remove brain signal
        if algp.updateBath
        ...
        fBrain = f_update_brain(dataBrain,Ac,theta,acqp.L(repmat(algp.maskbrain(:),[1 Nc]),:),Gbrain);
...
% multiply sensitivities back into bath body coil image to get final fBath
if ~isfield(algp,'sens')
    f = embed(fBrain,repmat(algp.maskbrain,[1 1 Nc])) + embed(fBath,repmat(algp.maskbath,[1 1 Nc]));
else
    f = embed(fBrain,repmat(algp.maskbrain,[1 1 Nc])) + algp.sens.*repmat(embed(fBath,algp.maskbath),[1 1 Nc]);
end
...
%
function [Gbrain,Gbath,Gall,GbathSENSE] = buildG(Nc,k,fov,mask,algp)
GbathSENSE = [];
if ~islogical(k) % non-cartesian
    ...
    if size(k,3) == 1 % 1 shot
        ...
        for ii = 1:nshot % build a system matrix for each shot
            Gbrainsub{ii} = Gmri(k(:,:,ii),algp.maskbrain,'fov',fov,'basis',{'dirac'});
            Gbathsub{ii} = Gmri(k(:,:,ii),algp.maskbath,'fov',fov,'basis',{'dirac'});
    ...
    Gbrain = Gmri_cart(k,algp.maskbrain);
    Gbath = Gmri_cart(k,algp.maskbath);
    Gall = Gmri_cart(true(size(k)));
        
end

if Nc > 1 % multiple coils; replicate the nufft's into a block-diag matrix
    if ~islogical(k) % non-cartesian % need to extend G obj to brain/bath/all
        for ii = 1:Nc
            tmp{ii} = G;
        end
        ...
        for ii = 1:Nc
            tmpbrain{ii} = Gbrain;
            tmpbath{ii} = Gbath;
            sensTmp = algp.sens(:,:,ii);
            tmpbathSENSE{ii} = block_fatrix({Gbath,diag_sp(col(sensTmp(algp.maskbath)))},'type','mult');
            tmpall{ii} = Gall;
        end
        Gbrain = block_fatrix(tmpbrain,'type','diag');
        Gbath = block_fatrix(tmpbath,'type','diag');
        GbathSENSE = block_fatrix(tmpbathSENSE,'type','col');
        Gall = block_fatrix(tmpall,'type','diag');
    end
end
...
Nc = length(fBrain)/length(theta);

% get total estimated image
err = data(:) - Gbrain*(fBrain.*repmat(exp(1i*(Ac+theta)),[Nc 1]));
...
Nc = length(f)/length(theta);
% data fidelity derivatives
img = f.*repmat(exp(1i*(Ac+theta)),[Nc 1]);
%g = real(sum(reshape(1i*conj(img).*(G'*(data(:) - G*img)),[length(theta) Nc]),2));
% g = 1i*conj(img).*(G'*(data(:) - G*img));
g = 1i*sum(reshape(conj(img).*(G'*(data(:) - G*img)),[length(theta) Nc]),2);
if ~algp.jointl1
...
if cost - costt >= algp.stepThresh*cost
    breakOut = false;
else
    breakOut = true;
end
...
Ac = A*c;
img = f.*repmat(exp(1i*(Ac+theta)),[Nc 1]);
g = A'*real(sum(reshape(1i*conj(img).*(G'*(data(:) - G*img)),...
    [length(theta) Nc]),2));
...
function [t,breakOut] = stepcalc_f(data,f,G,bathinit,algp,fgrad,dir,tmax)

%Nc = length(f)/sum(algp.maskbath(:));%size(data,2);
% use boyd's backtracking line search, which usually requires fewer cost evaluations

% calculate current cost
err = data(:)-G*f(:);
tmppenal = 0;
cost = 1/2*real(err'*err) + 1/2*algp.bathPenalty*real((f(:) - bathinit(:))'*(f(:) - bathinit(:)));
...
while (costt > cost + a*t*real(fgrad'*dir)) && t > 10^-6
    ...
    costt = 1/2*real(err'*err) + 1/2*algp.bathPenalty*real((ft(:) - bathinit(:))'*(ft(:) - bathinit(:))); 
...
switch algp.bathRecon
    ...
    case 'NLCG'
        ...
        for nn = 1:algp.fBathIters
            ...
            [t,breakOut] = stepcalc_f(data,f,G,bathinit,algp,g,dir,10000);
            if breakOut == true;break;end
            f = f + t*dir;
