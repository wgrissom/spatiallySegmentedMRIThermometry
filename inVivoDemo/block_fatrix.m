...
case 'mult'
	ob = block_fatrix_mult(blocks, arg);
otherwise
...
% block_fatrix_mult()
%
function ob = block_fatrix_mult(blocks, arg)

tmpsz = size(blocks{1});
%arg.dim = [size(blocks{1},1) size(blocks{end},2)];
arg.dim = [tmpsz(1) size(blocks{end},2)];
% check to make sure inner dims agree
for ii=1:length(blocks)-1
  tmpsz = size(blocks{ii});
  %if size(blocks{ii},2) ~= size(blocks{ii+1},1)
  if tmpsz(2) ~= size(blocks{ii+1},1)
    error 'dimension mismatch between subsequent blocks'
  end
end
...
% build Fatrix object
%
ob = Fatrix(arg.dim, arg, 'caller', 'block_fatrix(mult)', ...
	'forw', @block_fatrix_mult_forw, 'back', @block_fatrix_mult_back);

function y = block_fatrix_mult_forw(arg, x)

y = arg.blocks{end}*x;

for ii = length(arg.blocks)-1:-1:1
  y = arg.blocks{ii}*y;
end
  
function x = block_fatrix_mult_back(arg, y)
